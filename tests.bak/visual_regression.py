...
def run(update=False):
    ...
    with tempfile.TemporaryDirectory(prefix='einvite-visual-') as data:
        env={**os.environ,'EINVITE_DATA_DIR':data,'EINVITE_DEV_AUTH_TOKENS':'0'};proc=subprocess.Popen([sys.executable,'-u','server.py','--port',str(PORT)],cwd=ROOT,env=env,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
        try:
            wait()
            with sync_playwright() as p:
                ...
                try:
                    ...
                    account=response.json();assert 'token' not in account, 'Production visual test must use the HttpOnly cookie session'
                    invite=page.request.post(BASE+'/api/invitations',data={'slug':'visual-review','document':{'eventType':'Wedding','fields':{'names':'Visual Review','date':'2026-12-27','venue':'Grand Ballroom','message':'Join us for a beautiful celebration.'},'objects':{},'designPages':[],'sectionOrder':['schedule','venue','rsvp'],'settings':{'rsvpEnabled':True}}}).json()
                    active_id=invite.get('id','')
                except Exception:
                    active_id=''
                page.add_init_script(f"localStorage.removeItem('sovan-auth-token');const active={json.dumps(active_id)};if(active)localStorage.setItem('sovan-active-invite',active)")
