...
def main()->int:
 ...
 webgl=text(text('src/js/webgl-scene-backend-v22.js'));assert 'style.remove()' not in webgl and "createElement('style')" not in webgl and 'createElement("style")' not in webgl
 assert all(event in webgl for event in ('pointercancel','lostpointercapture','visibilitychange','blur'))
 adaptive=text(text('src/js/adaptive-gpu-quality-v22.js'));assert "createElement('style')" not in adaptive and 'createElement("style")' not in adaptive
