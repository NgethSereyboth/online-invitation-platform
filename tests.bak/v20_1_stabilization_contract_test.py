...
def main():
 contract=json.loads((ROOT / "docs" / "typography-contract.json").read_text())
 ...
 dashboard=(ROOT / "src" / "js" / "dashboard.js").read_text()
 ...
 polish=(ROOT / "src" / "js" / "final-polish.js").read_text();assert "!art.closest('.invite-card')" in polish
 ...
 editor=(ROOT / "src" / "js" / "typography-editor-v20.js").read_text();refresh=editor[editor.index('function refresh()'):]
 ...
 layout=(ROOT / "src" / "js" / "typography-layout-service.js").read_text();assert "return null" in layout[layout.index('function effectiveBackground'):layout.index('function diagnose')]
 assert 'contrast-undetermined' in layout
 public=(ROOT / "src" / "js" / "public-page.js").read_text();assert 'projectTypography=async lang' in public and 'ensureReady?.([model.font])' in public and 'locale:lang' in public
 req=(ROOT/'docs/requirements-test.txt').read_text().lower();assert 'fonttools' in req and 'brotli' in req
 ...
 release=(ROOT/'src'/'python'/'release_check.py').read_text();runner=(ROOT/'src'/'python'/'run_review_checks.py').read_text()
