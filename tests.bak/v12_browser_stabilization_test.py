...
def inline_public_html():
    html = (ROOT / "src" / "html" / "public.html").read_text(encoding="utf-8")
...
def main():
    ...
    with sync_playwright() as p:
        ...
        dashboard_html = (
            "<!doctype html><html><head><style>" + (ROOT / "src" / "css" / "tokens.css").read_text(encoding="utf-8") + "</style><style>" +
            (ROOT / "src" / "css" / "theme-hardening.css").read_text(encoding="utf-8") + "</style><style>" +
            (ROOT / "src" / "css" / "organized" / "dashboard-empty-state.css").read_text(encoding="utf-8") +
            "</style></head><body><div id='inviteGrid'></div><button id='newBtn'></button><script>" +
            (ROOT / "src" / "js" / "dashboard-empty-state.js").read_text(encoding="utf-8") + "</script></body></html>"
        )
        ...
        templates_html = template_shell + "<script>" + fetch_stub + "</script><script>" + (ROOT / "src" / "js" / "builtin-templates.js").read_text(encoding="utf-8") + "</script><script>" + (ROOT / "src" / "js" / "templates.js").read_text(encoding="utf-8") + "</script></body></html>"
