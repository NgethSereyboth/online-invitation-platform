...
def run() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="e-invitation-website-smoke-") as data_dir:
        ...
        try:
            ...
            # Context-aware AI assistant local fallback task aliases.
            _, ai_formal = request(base, "/api/ai/assist", "POST", {"task": "rewrite-formal", "prompt": "Please join us", "context": {"names": "Smoke Couple", "eventType": "Wedding", "venue": "Test Venue"}}, token, 200)
            assert ai_formal["provider"] == "template" and ai_formal["providerMode"] == "offline" and "Smoke Couple" in ai_formal["text"]
            _, ai_khmer = request(base, "/api/ai/assist", "POST", {"task": "translate-khmer", "prompt": "Join us", "context": {"namesKm": "សុភា និង ដារ៉ា", "eventType": "Wedding"}}, token, 200)
            assert "សូមគោរពអញ្ជើញ" in ai_khmer["text"]
            _, ai_outline = request(base, "/api/ai/assist", "POST", {"task": "page-outline", "prompt": "", "context": {}}, token, 200)
            assert "Opening" in ai_outline["text"] and "Venue" in ai_outline["text"]
