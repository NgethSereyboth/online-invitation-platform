...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="einvite-env-generator-") as folder:
        ...
        command = [sys.executable, str(ROOT / "src" / "python" / "build" / "prepare_production_env.py"), "--public-url", "https://events.example.test", "--output", str(generated_path), "--trusted-proxy-ips", "172.20.0.10"]
    ...
    dockerignore = (ROOT / "deploy" / ".dockerignore").read_text(encoding="utf-8")
    ...
    checklist = (ROOT / "docs" / "PRODUCTION_LAUNCH_CHECKLIST.md").read_text(encoding="utf-8")
    assert all(marker in checklist for marker in ("restore drill", "/api/health/ready", "rollback", "private bucket"))
    test_requirements = (ROOT / "docs" / "requirements-test.txt").read_text(encoding="utf-8")
    ...
    compose = (ROOT / "deploy" / "docker-compose.production.example.yml").read_text(encoding="utf-8")
