...
def resolve_repo_asset(relative: str) -> Path:
    for candidate in (ROOT / relative, ROOT / "src" / "js" / relative, ROOT / "src" / "css" / relative, ROOT / "src" / "html" / relative):
        if candidate.exists():
            return candidate
...
window.fetch=async()=>({ok:false,status:404,statusText:'Not Found',json:async()=>({}),text:async()=>'',headers:new Headers()});
    ...
    def inline_css(match: re.Match[str]) -> str:
        path = resolve_repo_asset(match.group(1))
        if not path.exists():
