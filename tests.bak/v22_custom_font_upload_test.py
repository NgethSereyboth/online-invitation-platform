...
def test_integration_contracts():
 routes=json.loads((ROOT / "docs" / "route-bundle-sources-v15.json").read_text())
 ...
 bootstrap=(ROOT / "src" / "js" / "editor-deferred-tools-bootstrap-v0_52.js").read_text();assert 'font-browser-loader-v22.js' in bootstrap
 loader=(ROOT / "src" / "js" / "font-browser-loader-v22.js").read_text();assert 'custom-fonts-v22.js' in loader and 'font-browser.js' in loader and 'custom-fonts-v22.css' in loader
 upload=(ROOT / "src" / "js" / "upload-client.js").read_text();assert 'uploadFont' in upload and '/fonts' in upload
 browser=(ROOT / "src" / "js" / "font-browser.js").read_text();assert '.ttf,.tff,.otf,.woff2' in browser and 'licenseAcknowledged' in browser
 server_text=(ROOT/'src'/'python'/'server.py').read_text();assert 'optimize_custom_font' in server_text and 'def upload_font' in server_text
 model=(ROOT / "src" / "js" / "typography-document-model.js").read_text();assert 'normalizeDocumentFonts' in model
