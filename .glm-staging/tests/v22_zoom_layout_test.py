...
css=(ROOT / "src" / "css" / "zoom-layout-v22.css").read_text(encoding='utf-8');manifest=json.loads((ROOT / "docs" / "route-bundle-sources-v15.json").read_text(encoding='utf-8'))
...
compact=(ROOT / "src" / "css" / "zoom-layout-pages-v22.css").read_text(encoding='utf-8');assert 'compact authenticated-page zoom support' in compact and '--v22-control-min:44px' in compact
for page,cfg in manifest['pages'].items():
 ...
 bundle=ROOT/"src"/"css"/f"bundle-{Path(page).stem}-v15.css";assert bundle.exists(),bundle
