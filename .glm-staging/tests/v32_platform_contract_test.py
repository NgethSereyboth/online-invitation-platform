...
for name in ('platform_v32/schema.py','platform_v32/storage.py','platform_v32/jobs.py','platform_v32/service.py','src/python/platform_worker_v32.py','src/python/platform_scheduler_v32.py','deploy/.env.example','deploy/docker-compose.production.example.yml'):assert (ROOT/name).is_file(),name
