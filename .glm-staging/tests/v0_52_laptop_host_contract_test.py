...
batch=(ROOT/'scripts'/'HOST_EINVITE_ON_LAPTOP.bat').read_text(encoding='utf-8')
script=(ROOT/'scripts'/'host-einvite-laptop.ps1').read_text(encoding='utf-8')
guide=(ROOT/'docs'/'LAPTOP_HOSTING.md').read_text(encoding='utf-8')
...
result=subprocess.run(['powershell.exe','-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',str(ROOT/'scripts'/'host-einvite-laptop.ps1'),'-CheckOnly','-SkipFirewall','-NoBrowser','-AllowUploadsWithoutMalwareScan','-Port','18080'],cwd=ROOT,text=True,capture_output=True,encoding='utf-8',errors='replace',timeout=30)
