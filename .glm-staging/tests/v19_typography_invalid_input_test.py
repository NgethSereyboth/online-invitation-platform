...
def load_server():
 import sys;sys.path.insert(0,str(ROOT));sys.path.insert(0,str(ROOT/'src'/'python'))
 spec=importlib.util.spec_from_file_location('einvite_server_v19_1',ROOT/'src'/'python'/'server.py');assert spec and spec.loader
...
def main()->int:
 ...
 ''')
 out=json.loads(subprocess.run(['node','-e',script],cwd=ROOT/'src'/'js',text=True,capture_output=True,check=True).stdout)
 for row in out['rows']:
