...
def run() -> None:
    with tempfile.TemporaryDirectory(prefix="einvite-signed-upload-") as data_dir:
        ...
        try:
            ...
            # Even with object storage configured, editor-facing material URLs stay
            # behind the first-party authorization-aware media gateway.
            assert completed["url"].startswith("/uploads/")
            assert completed["responsiveBase"].startswith("/api/image/")
