...
for name in ('adaptive-templates-v25.js','studio-governance-v25.js','print-readiness-v25.js','template-bindings-v25.js'):
 text=(ROOT/'src'/'js'/name).read_text(encoding='utf-8');assert 'addEventListener(\'keydown\'' not in text and 'addEventListener("keydown"' not in text,name
for command in ('templates.adaptive','studio.governance','print.preflight','templates.refreshBindings'):
 assert command in ''.join((ROOT/'src'/'js'/n).read_text(encoding='utf-8') for n in ('adaptive-templates-v25.js','studio-governance-v25.js','print-readiness-v25.js','template-bindings-v25.js')),command
...
assert 'contentBinding' in (ROOT / "src" / "js" / "adaptive-templates-v25.js").read_text(encoding='utf-8')
assert 'bindingDetached' in (ROOT / "src" / "js" / "template-bindings-v25.js").read_text(encoding='utf-8')
