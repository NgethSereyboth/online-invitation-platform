...
install_system_packages() {
  ...
  case "$pkg_mgr" in
    apt)
      if [[ $EUID -ne 0 ]]; then
        log_error 'Use sudo with --install-system-packages.'
        exit 1
      fi
      apt-get update -qq
...
verify_installation() {
  ...
  # Compile check
  if [[ -f server.py ]]; then
    if ! .venv/bin/python -m py_compile server.py; then
  ...
  if [[ -f production_preflight.py ]]; then
    .venv/bin/python production_preflight.py --env-file .env.production --check-dependencies || true
  fi
