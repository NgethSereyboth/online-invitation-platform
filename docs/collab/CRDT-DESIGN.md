...
### V54.30 vendoring milestone (ROADMAP-V2 §4.1) — DONE

Y.js is now vendored + wired into the editor. The previous Phase 4b
deliverable (V54.7) shipped the V52 modules but left the actual Y.js
files un-downloaded; the editor's `<script>` tags pointed at `/vendor/yjs/y.js`
which would have 404'd at runtime. The V54.30 task closes that gap.

What was done in V54.30 (see `vendor/yjs/README.md` for the full details):

1. **`bash vendor/yjs/download.sh`** — DONE. The script was rewritten to
   fetch from `esm.sh/yjs@13.6.27/es2022/yjs.bundle.mjs` (the original
   Phase 4b download.sh assumed `dist/y.js` existed on unpkg/jsdelivr —
   it does NOT for Y.js 13.6.x, which ships ESM-only). The esm.sh
   `?bundle` parameter produces a self-contained ESM bundle with lib0
   inlined; the only external import (`/node/process.mjs`) is rewritten
   to `/vendor/yjs/process-stub.js` (a tiny stub that returns an empty
   object — lib0's process probes return undefined, which it handles
   gracefully in the browser).
2. **SRI integrity hashes** — DONE. `vendor/yjs/INTEGRITY.txt` is
   auto-generated by `download.sh` with sha384 hashes for `y.js`,
   `y-indexeddb.js`, `process-stub.js`, and `lib0.js`.
3. **Script tags in `src/html/designer.html` + `src/html/dashboard.html`** — DONE.
   The HTML files include:
   - `<script src="/vendor/yjs/lib0.js" integrity="sha384-...">` (no-op
     marker — lib0 is bundled INTO y.js, so loading it does nothing).
   - `<link rel="modulepreload" href="/vendor/yjs/y.js" integrity="sha384-...">`
     (SRI-enforced preload of the Y.js ESM bundle).
   - `<link rel="modulepreload" href="/vendor/yjs/process-stub.js" integrity="sha384-...">`
     (SRI-enforced preload of the process stub).
   - `<link rel="preload" href="/vendor/yjs/y-indexeddb.js" integrity="sha384-..." as="fetch">`
     (vendored but NOT executed at runtime — our `crdt-yjs-indexeddb.js`
     implements its own IndexedDB persistence using native APIs +
     `Y.applyUpdate`).
   - An inline `<script type="module">` that imports `Y` from
     `/vendor/yjs/y.js` and assigns it to `window.Y` (so the classic-script
     V52 modules can read it lazily at init time).
   - An inline classic `<script>` probe that waits for the
     `einvite:yjs-loaded` event (or 8s timeout) and logs Y.js load
     success / failure to the browser console.
4. **`sync_frontend_assets.py` mirrors `vendor/yjs/`** — DONE (was
   already in place from V54.7 — the SYNC_PLAN includes the recursive
   mirror entry `(VENDOR_DIR, SCRIPT_DIR / "vendor", "**/*", True)`).
5. **`window.Y` defined on editor load** — DONE. Verified end-to-end by
   spinning up the real HTTP server (`tests/v14_test_utils.app_server`)
   and fetching `/vendor/yjs/y.js` → HTTP 200 with
   `Content-Type: text/javascript` (a valid ES module MIME type per the
   HTML spec).
6. **V52 modules initialize without "window.Y missing" error** — DONE.
   The V52 modules (`crdt-yjs-indexeddb.js`, `crdt-yjs-undo.js`,
   `crdt-yjs-rich-media.js`) all read `window.Y` lazily inside functions
   (open, constructor, factory), NOT at IIFE-evaluation time. They throw
   the documented `'Y.js library (window.Y) is not loaded — see
   vendor/yjs/README.md'` error if Y.js is missing at init time —
   verified by `tests/v52_yjs_wiring_test.py` Phase E.

What was NOT done in V54.30 (deferred to V54.8 / Phase B):

- The Y.js dual-write path (`EINVITE_COLLAB_V52_DUALWRITE=1`) is still
  opt-in and the per-op translator (Y.js binary → V31 JSON ops) is still
  a coarse approximation that records the entire document snapshot as a
  single V31 'set' op per Y.js update. The real per-op translator ships
  in Phase B (V54.8).
- The headless browser test (Playwright) that opens two editor tabs and
  verifies a text edit in one appears in the other via real Y.js CRDT
  merge semantics — the existing `tests/v52_crdt_offline_merge_test.py`
  uses a dependency-free YjsShim binary encoder (NOT the real Y.js
  library), so it verifies the BACKEND contract but not the actual CRDT
  merge semantics. That test is Phase B work.

### Phase A — Dual-write (current commit, V54.7 + V54.30 vendored)
...
The 5-step procedure for migrating an eInvite deployment from V31-only
to V52-only. Steps 1–4 are completed in V54.30 + V54.31; step 5 is pending
Phase B (V54.8+) — needs 30 days of production traffic on dual-write first.

| Step | Action | Status | Phase | Owner |
|---|---|---|---|---|
| 1 | Run `bash vendor/yjs/download.sh` to fetch the pinned Y.js + y-indexeddb + lib0 + process-stub files and compute SRI hashes. | ✅ DONE (V54.30) | 4.1 | maintainer |
| 2 | Verify `window.Y` is defined in the editor by loading `/designer.html` or `/dashboard.html` and checking the browser console for `[V54.30] Y.js loaded successfully (NN exports)`. | ✅ DONE (V54.30) | 4.1 | maintainer |
| 3 | Set `EINVITE_COLLAB_V52_DUALWRITE=1` in the dev environment (`platform_v32/config.py` reads this env var). New Y.js updates will be dual-written to both the V52 binary table AND the V31 JSON `draft_json` column. | ✅ DONE (V54.31) | 4.2 (WEEK 6) | phase-4b maintainer |
| 4 | Verify dual-write works: open an invitation with existing V31 history; verify the Y.Doc is seeded correctly; make an edit; verify both the V31 row and the V52 row are written; verify a second client joining sees the V52 state. | ✅ DONE (V54.31 — `tests/v52_dualwrite_migration_test.py` covers: V31 update → V52 snapshot migratedFrom='v31' marker; V52 update → dualWrite=True response + V31 row inserted with `yjsDualWrite=True` payload marker; idempotency on duplicate re-post) | 4.2 (WEEK 6) | phase-4b maintainer |
| 5 | Flip to V52 authoritative: new sessions join via `/collaboration/v52/snapshot` only. The editor reads/writes through the Y.js doc directly (no V31 mirror). Existing sessions continue on V31 until they reload. | ⏳ pending (Phase B / V54.8+) | 4.2 follow-up | phase-4b maintainer |

The dual-write window (steps 3–4) must run for at least 30 days of
production traffic on V52 with no merge-corruption incidents before
step 5 is taken. See Phase B's acceptance gate above for the full
criteria.
