...
-- ── V54.32 (phase-2c — ROADMAP-V2 §4.3) ───────────────────────────────────
-- Calendar integrations + venue maps + gift registry.
-- (1) venue geocoding cache on invitations (lat / lng / geocoded_at — stays
--     NULL until the first public-page hit to /api/invitations/{id}/venue-geocode
--     proxies the venue string to OpenStreetMap Nominatim).
-- (2) gift_registry_items + gift_registry_claims — host-curated list with
--     a claim/cancel lifecycle mirroring signup_sheets / signup_claims.
...
ALTER TABLE invitations ADD COLUMN IF NOT EXISTS venue_geocoded_at BIGINT;
...
CREATE INDEX IF NOT EXISTS idx_gift_registry_items_invitation
  ON gift_registry_items(invitation_id, archived_at, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_gift_registry_claims_item
  ON gift_registry_claims(item_id, cancelled_at, created_at DESC);
