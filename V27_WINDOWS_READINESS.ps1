...
try{$test=Join-Path $Root 'data\.v27-write-test';New-Item -ItemType Directory -Force -Path (Split-Path $test)|Out-Null;Set-Content $test 'ok';Remove-Item $test -Force;Add-Result 'Data write permission' $true (Join-Path $Root 'data')}catch{Add-Result 'Data write permission' $false $_.Exception.Message}
try{$backup=Join-Path $Root 'data\backups\.v27-backup-test';New-Item -ItemType Directory -Force -Path (Split-Path $backup)|Out-Null;Set-Content $backup 'ok';Remove-Item $backup -Force;Add-Result 'Backup write permission' $true (Join-Path $Root 'data\backups')}catch{Add-Result 'Backup write permission' $false $_.Exception.Message}
$portBusy=$false
...
if($failed.Count){Write-Host "`nV27 Windows readiness needs attention in $($failed.Count) area(s)." -ForegroundColor Yellow;exit 1}
Write-Host "`nV27 Windows readiness passed." -ForegroundColor Green
