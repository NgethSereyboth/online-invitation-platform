...
})();;const $=s=>document.querySelector(s),inviteId=window.EInviteContext?.getInvitationId()||'demo-wedding';localStorage.removeItem('sovan-auth-token');let responses=[],wishes=[],documentData={},server=false,rsvpEnabled=true;
...
function metrics(){if(!rsvpEnabled){$('#metrics').innerHTML=`<div class="response-metric"><strong>Disabled</strong>RSVP status</div><div class="response-metric"><strong>${responses.length}</strong>Historical responses</div><div class="response-metric"><strong>${wishes.length}</strong>Guest wishes</div>`;return}const yes=responses.filter(r=>String(r.status).startsWith('Yes')),no=responses.filter(r=>String(r.status).startsWith('Unable')),maybe=responses.filter(r=>r.status==='Maybe'),guests=yes.reduce((n,r)=>n+Number(r.guest_count||r.count||1),0);$('#metrics').innerHTML=`<div class="response-metric"><strong>${responses.length}</strong>Responses</div><div class="response-metric"><strong>${yes.length}</strong>Attending replies</div><div class="response-metric"><strong>${guests}</strong>Attending guests</div><div class="response-metric"><strong>${wishes.length}</strong>Guest wishes</div>`}
...
async function load(){if(window.EInviteBackend?.ready)await window.EInviteBackend.ready;if(window.EInviteBackend&&!window.EInviteBackend.isAvailable()){server=false;documentData=JSON.parse(localStorage.getItem(`sovan-invite-draft-v3:${inviteId}`)||'{}');responses=JSON.parse(localStorage.getItem(`sovan-invite-rsvps-v3:${inviteId}`)||'[]');wishes=JSON.parse(localStorage.getItem(`sovan-invite-wishes-v1:${inviteId}`)||'[]');window.EInviteBackend.message($('#rsvpPanel'),'RSVP responses and guest wishes require the full application server.');metrics();renderWishes();return}try{await request('/api/health');server=true;const invite=await request(`/api/invitations/${inviteId}`);documentData=invite.document||{};rsvpEnabled=documentData.settings?.rsvpEnabled!==false;[responses,wishes]=await Promise.all([request(`/api/invitations/${inviteId}/rsvps`),request(`/api/invitations/${inviteId}/wishes`)]);}catch{server=false;documentData=JSON.parse(localStorage.getItem(`sovan-invite-draft-v3:${inviteId}`)||'{}');responses=JSON.parse(localStorage.getItem(`sovan-invite-rsvps-v3:${inviteId}`)||'[]');wishes=JSON.parse(localStorage.getItem(`sovan-invite-wishes-v1:${inviteId}`)||'[]')}metrics();renderResponses();renderWishes()}
load();;(function(){
'use strict';
