...
(async()=>{if(!token){status.textContent='No verification token was provided.';return}try{const response=await fetch('/api/auth/verification/confirm',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({token})}),data=await response.json().catch(()=>({}));if(!response.ok)throw Error(data.error||'Verification failed');status.textContent='Your email address is now verified.'}catch(error){status.textContent=error.message}})();;(function(){
'use strict';
