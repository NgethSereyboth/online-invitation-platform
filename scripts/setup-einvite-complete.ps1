...
function Test-PythonExecutable {
    ...
    try {
        ...
        $output = & $Command @PrefixArgs -c "import sys; assert sys.version_info >= (3, 10), 'Python 3.10 or newer is required'; print(sys.executable)" 2>$null
        $code = $LASTEXITCODE
...
function Test-WindowsServer {
    try {
        ...
        return [bool]($productName -match 'Server')
    } catch {
        return $false
    }
}
...
try {
    ...
    Write-Step '1/8 - Checking Windows and available installers'
    $wingetAvailable = Test-Command 'winget'
    $windowsServer = Test-WindowsServer
    if ($wingetAvailable) {
        Write-Log 'winget is available.' Green
    } else {
        Write-Log 'winget is unavailable. Existing software will be used; missing applications must be installed manually.' Yellow
    }
    if ($windowsServer) {
        Write-Log 'Windows Server detected. Docker Desktop installation will be skipped; use native Windows hosting or Docker on a Linux server.' Yellow
    }
    ...
    if (-not $detectedPython) {
        if (-not $wingetAvailable) {
            throw 'Python 3.10 or newer was not found and winget is unavailable. Install 64-bit Python from python.org, enable Add Python to PATH, then rerun this setup.'
        }
    ...
    if ($wingetAvailable) {
        ...
        if (-not $SkipDocker -and -not $windowsServer) {
        ...
        Write-Log 'Optional Git, Node.js, cloudflared, and Docker installation was skipped because winget is unavailable.' Yellow
    }
